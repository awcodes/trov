...
class MediaObserver
{
    ...
    public function created(Media $media)
    {
        ...
        if ($media->disk == 'cloudinary') {
            ...
            $uploadedFile = $file->storeOnCloudinaryAs(config('trov.media.directory'), $media->filename);
    ...
    public function deleted(Media $media)
    {
        ...
        if ($media->disk !== 'cloudinary') {
            ...
            foreach (config('trov.media.sizes') as $name => $data) {
    ...
    {
        $pathinfo = pathinfo($media->filename);
        foreach (config('trov.media.sizes') as $name => $mediaSize) {
