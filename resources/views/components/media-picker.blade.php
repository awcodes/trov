<div x-data="{
        ...
        getFiles: async function(url = '/trov/media') {
        ...
        searchFiles: async function(event) {
            this.isFetching = true;
            const response = await fetch('/trov/media/search?q=' + event.target.value);
            const result = await response.json();
    ...
    x-on:close-modal.window="if ($event.detail.id === 'trov-media-picker') isOpen = false; resetPicker();"
    x-on:open-modal.window="if ($event.detail.id === 'trov-media-picker') isOpen = true; fieldId = $event.detail.fieldId; getFiles();"
    ...
    aria-labelledby="trov-media-modal-heading"
    role="dialog"
    aria-modal="true"
    class="inline-block trov-media-picker-modal"
    x-id="['tab']">
    ...
    <div x-show="isOpen"
        ...
        <button x-on:click="isOpen = false; resetPicker();"
            type="button"
            aria-hidden="true"
            class="fixed inset-0 w-full h-full bg-black/50 focus:outline-none trov-media-picker-modal-close-overlay"></button>
        ...
        <div x-show="isOpen"
            ...
            <div @class([
                'w-full mx-auto p-2 space-y-2 bg-white rounded-xl flex flex-col cursor-default h-full trov-media-picker-modal-window',
                'dark:bg-gray-800' => config('filament.dark_mode'),
            ])>
                <div id="trov-media-modal-heading"
                    class="flex items-center justify-between px-4 py-1 trov-media-picker-modal-header">
                ...
                <div class="flex-1 space-y-2 overflow-hidden trov-media-picker-modal-content">
                ...
                <div class="flex items-center justify-end px-4 py-2 trov-media-picker-modal-footer">
                    <x-filament::button type="button"
                        color="success"
                        x-bind:disabled="!selected"
                        x-on:click="$dispatch('close-modal', {id: 'trov-media-picker', media: selected, fieldId: fieldId})">
                        Use Selected Image
                    </x-filament::button>
                </div>
            </div>
